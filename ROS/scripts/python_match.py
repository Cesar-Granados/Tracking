...
def conversion_matriz():
    #|---- local var ----|#
    file = r_archivo("/home/cesar/catkin_ws/src/tracking/recursos/Descriptores/Botella.txt")
    #file = r_archivo("./recursos/Descriptores/Botella.txt")
    file2 = r_archivo("/home/cesar/catkin_ws/src/tracking/recursos/Descriptores/NoBotella.txt")
...
def seleccionar_object(objeto,img):
    ...
    #|---- local var ----|#
    botella = cv2.imread("../recursos/Imagenes-Objeto/Botella/img6.png")
    hist_botella = []
    res_hist = []
    #|---- Diff Histogram ----|#
...
def listener():
	...
	rospy.Subscriber("camara", String, callback)
	print("Recibiendo la imagen a procesar.")
	return String

def talker():
	#rospy.init_node('talker', anonymous=True)
	pub = rospy.Publisher('distancia', String, queue_size=10)
	rate = rospy.Rate(10) # 10hz
	if not rospy.is_shutdown():
		hello_str = "Enviando imagen. %s" % rospy.get_time()
		rospy.loginfo(hello_str)
		pub.publish(hello_str)#|-- Publicar informacion relevante. --|#
		rate.sleep()
...
if __name__ == '__main__':
	cap1 = listener()
	#|---------------------------------------------------|#
	#cap = cv2.VideoCapture("../recursos/Videos/Oorion1.avi")
	cap = cv2.VideoCapture("/home/cesar/catkin_ws/src/tracking/recursos/Videos/Oorion1.avi")
	orb = cv2.ORB_create()
	ms = MeanShift()
	#|---------------------------------------------------|#
	trainData, responses = conversion_matriz()
	knn = entrenar_knn(trainData, responses)
	#images_, kpoints_, descriptors_ = cargar_carac("../recursos/Imagenes-Objeto/Orientaciones/img",orb)
	images_, kpoints_, descriptors_ = cargar_carac("/home/cesar/catkin_ws/src/tracking/recursos/Imagenes-Objeto/Orientaciones/img",orb)
	#|---------------------------------------------------|#
