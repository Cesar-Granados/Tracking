...
def listener():
	...
	try:	
		...
		rospy.Subscriber("/videofile/image_raw/compressed", CompressedImage, callback4, [knn, track_window, roi_hist, term_crit, control, cluster, 0], queue_size=1)
		#rospy.Subscriber("/frame_now/compressed", CompressedImage, callback5, queue_size=1)
...
def callback4(data, args):
	...
	if img_np is not None:
		...
		string0 = "camara: "+str(pose_camara.position.x)+","+str(pose_camara.position.y)+","+str(pose_camara.position.z)
		string1 = "objeto: "+str(centro[0])+","+str(centro[1])+","+str(centro[2])
...
def talker(posicion):
	#|--- Generar publisher ---|#
	pub = rospy.Publisher('Distancia', PoseStamped, queue_size=1)
	#rate = rospy.Rate(1) # 10hz
	if not rospy.is_shutdown():
		#|--- Publicar resultado ---|#
		pose = PoseStamped()
		pose.header.stamp = rospy.Time.now()
		pose.header.frame_id = "ground"
...
def meanshift_cl(kp, des, img):
	...
	#|--- Eliminar cluster que no aportan informacion al sistema ---|#
        if cluster[7] < 50 or cluster[4] <= 15 or cluster[5] <= 15:
            continue
...
def k_nearest(knn, cluster):
    botella = []
    net = NetworkReader.readFrom('/home/cesar/catkin_ws/src/tracking/src/RedNeuronalInt 40-80(0.4-0.95).xml')
    for i in cluster:
        imagen = i[10]
        hu = Hu(imagen)
	respuesta,no_datos = 0, 0
	for k,j in enumerate(i[9]):
		data = np.append(hu,j)
        	respuesta = respuesta + net.activate(data)
		no_datos = k
    ...
    for i in cluster:
        ...
        #|--- aplicar knn ---|#
        ret, results, neighbours, dist = knn.findNearest(newcomer, 50)
